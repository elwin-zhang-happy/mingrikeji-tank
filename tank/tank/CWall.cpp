...
bool CWall::CheckHitEx(const CBullet &blt, PointF *ptHit/*=nullptr*/, float *dir/*=nullptr*/, float *pDelay/*=nullptr*/) const
{
    ...
    RectF rc = GetRect();
    std::vector<std::tuple<CVector2D, float, float>> v;
    {
        //  左线
        {
            CLine line;
            ...
            if(game_hit::IsHit(ray, line, newSpeed, crossPoint, &fDelay)) {
                ...
                v.push_back(std::make_tuple(crossPoint, fDelay, theta));
            }
    ...
    if(!v.empty()) {
        ...
        if(ptHit) {
            ptHit->X = std::get<0>(*it).x;
            ptHit->Y = std::get<0>(*it).y;
        }
        if(pDelay) {
            * pDelay = std::get<1>(*it);
        }
